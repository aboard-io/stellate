...
// down, in app/audio/live.js's NO-ISOLATION FALLBACK — the WAV-FIRST route is chosen
// automatically. Nothing in this file touches the engine choice; it just plays.
import { S, set, subs, K, QSFLAGS } from "../core/state.js";
import { POS, WORLD_W, WORLD_H, MAP_CENTER } from "../core/world.js";
import { retarget } from "../audio/targeting.js";
import { clampZoom, drawMap, ZOOM } from "../map/starmap.js";
import { goLive, stopLive } from "../audio/live.js";
import { buildShareUrl } from "../core/share.js";
import { booted } from "../main.js";   // this entry runs ON TOP of the app: wait for its boot before framing
...
function centerOn(pt, k){
  const Z = ZOOM, svg = document.getElementById("map");
  if(!Z || !svg) return;
...
let tries=0;
(function ready(){
  if(S.playing && booted){ applyGenre(); fitBox(); syncOverlay(); return; }
