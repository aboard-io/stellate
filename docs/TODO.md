...
**NOT free — `dx7-presets.json`.** The 671 KB is the RAW size; the live server
sends it **gzipped at 74.7 KB** (`curl -H 'Accept-Encoding: gzip'` against
stellate.app, `content-encoding: gzip`). So the cut is worth ~224 KB of network
(three redundant fetches) plus three redundant 671 KB JSON parses on worker
threads — not the ~2 MB the raw number implies. Still ~6% of a 3.37 MB session,
so worth doing, but it is a coordinated async change across `main.js` →
`live.js` → `stream-worker.js` / `stem-worker.js` in the live audio path, and it
should be priced as that rather than as a freebie. Cheaper alternative worth
checking first: the file is served `cache-control: no-cache`, but it is
generated by `sysex2params.js` and changes only when the cartridge banks do —
if the HOSTING §5 mutable-carve-out can be narrowed to actual manifests, the
browser would serve three of the four fetches from cache with **zero** code
change.
