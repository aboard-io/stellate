...
- **`dx7-presets.json` Nx fetch — SOLVED BY THE CHEAP ROUTE; the code change is
  deliberately NOT done.** Three things the earlier estimate had wrong, all
  measured with a request-counting server that mirrors the production headers
  (`Cache-Control` + `ETag` + `Last-Modified`), driving a cold `index.html` +
  `__X.goLive()` in headless chromium:
  ...
  - The redundant fetches were never worth ~224 KB. `no-cache` means
    ...
    and came back **304 with no body** (confirmed against the live nginx with
    `If-None-Match`). The real cost was 2 round trips, not 149 KB.
  - It was never the HOSTING §5 mutable-JSON carve-out doing it. That rule is
    nested inside `location /found/` and reaches nothing else; `no-cache` on
    this file comes from the **server-wide** `add_header`, a default for code.
    `sw.js`'s `MUTABLE` regex is `found/`-scoped for the same reason and needed
    no change.

  The fix is one nginx `location =` block (`private, max-age=300`) written into
  HOSTING §5, plus a reminder line in the deploy smoke — **the nginx config is
  not in this repo, so a human must paste it on the droplet and in the
  aboardresearch alias.** Measured with it: **3 requests → 1**, zero code
  change, zero page errors. The remaining prize for the coordinated
  `main.js → live.js → workers` change is 2 × 8.4 ms of `JSON.parse` on worker
  threads, and `postMessage` would pay most of that back in structured-clone
  anyway — not worth putting a hand into the live audio path's init sequence.
...
Four cuts were listed as free. **Two are; two were mis-scoped, and the
measurements below are what changed the answer.**
...
- **`zones.json` + `_gm-extract-summary.json` dropped** — 134 files, 544 KB of
  ...
  `K.SAMPLERS`, and `faust/sf2.js` says so outright ("no audio path reads
  zones.json at render time").
- **`@grame` narrowed 28 MB → 194 KB.** The web needs exactly one file of that
  tree, `dist/esm/index.js`; `live.js` / `stream-worker.js` / `stem-worker.js`
  import `FaustWasmInstantiator` + `FaustMonoDspGenerator` from it to
  instantiate PRECOMPILED factories. The rest is the Faust compiler
  (libfaust-wasm 5.4 MB), the cjs/bundle builds (16.4 MB), tests, assets and a
  466 KB source map — all build-time. A smoke check now fetches that one file so
  a filter typo fails the deploy loudly instead of silently killing audio.

**`dx7-presets.json` — free after all, but not for the reason anyone thought,
and not by the route anyone proposed.** The 671 KB is the RAW size; the live
server sends it **gzipped at 74.7 KB**. It is fetched **3** times on a desktop
session (`app/main.js` at boot + one per stream-worker instance — `stem-worker`
has a fetch site but nothing spawns that worker any more), and because
`no-cache` means store-and-revalidate, fetches 2 and 3 already went out
conditional and came back **304, no body**. So the network prize was two round
trips, not ~224 KB, and the payload argument for the coordinated
`main.js → live.js → workers` change collapses with it: what is left is
2 × 8.4 ms of `JSON.parse` on worker threads, which `postMessage` would largely
pay back in structured-clone. **Not done, deliberately** — a hand in the live
audio path's init sequence costs more than it buys.
...
`found/`-scoped for the same reason, so neither needed narrowing. The block is
written into HOSTING §5 and the deploy smoke prints a reminder until it is
live, because **nginx config is not in this repo** — it is a hand-paste on the
droplet and in the aboardresearch alias.

**NOT free — the eager shell.** Two separate problems, neither one deferral:
- `engine/genre-verifier.js` **cannot** simply lazy-load: `app/state.js:9` binds
  `export const V=GenreVerifier` at module top level, and `rescore()` runs
  during boot (`app/main.js:46`) and every 4 bars. Deferring the script means
  deferring the score readout — a visible behaviour change, not a free win.
- `app/starcruise.js` **is** genuinely deferrable (48 KB gz, only needed on the
  aliens view) — but it registers `window.__STARCRUISE` as a side-effecting
  module at load, and ~12 browser gates reach that global after page load.
  Deferring it means updating every one of those to trigger the import first.
  Real work with a wide blast radius; not a free cut.
...
`tools/`, `test/`, the six HTML pages, `sw.js` and `verify.sh`. Includes the
named sub-items: `app/glyphs.js`'s `// ARCHAEOLOGY.` block; the four stale
`// faust/…` self-headers; the three comments describing things that aren't
there (`index.html` background chip, `readouts.js` "feature retired",
`starcruise.js` `makeShip()`); and the five-way *"preserved on branch
legacy-csound"* duplication (CLAUDE.md says it once).

**What stays, on purpose:**

- Author metadata, `© 2026 Paul Ford`, the visible "made by" credits and the
  `social-meta.test.js` assertions that enforce them — attribution, not
  provenance narration.
- Citations of docs that still exist (`docs/TIMING-AUDIT-2026-07.md`,
  `docs/ENGINE-AUDIT-2026-07.md`). Renaming a dated *filename* is Stage F.
- The archive.org license-verification date in `fetch-bed-expansion.sh` — that
  is a legal record of when the licences were checked, not a diary entry.
...
*inside* a template literal in `faust/sampler.js` that holds generated DSP source
— inert, and `strip-fuzz-test`'s fused-kernel drift gate replays it
bit-identical against `stripStepRef`.

`comment-only.js` was itself fixed during this pass: its walker knew about
strings but not regex literals, so a literal like `/['"]/` flipped it into
"inside a string" and it silently stopped stripping comments for the rest of the
file — reporting four clean files as changed. It now tracks regex literals and
character classes.

**Gates:** `verify.sh` 11/11 PASS · `matrix --no-cache` **274/274 diagonal
dominant** · `meter.test` byte-identity green · `strip-fuzz` bit-identical ·
`boot-smoke` 14/14 · `explorer-ui` · `genre-viz` · `midi-export` 36/36 · theory
· pipes · social-meta · doc-counts.

**Two PRE-EXISTING failures, confirmed identical on a stashed clean tree — not
from this pass, and still open:**
