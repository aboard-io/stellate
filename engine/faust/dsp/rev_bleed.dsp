...
// `rin = rev + d*0.2 + (ppl+ppr)*0.12` into its INTERNAL zita, but that zita is
...
//   Inputs: 0 delay send, 1 ping-pong send.  Output: d*0.2 + (ppl+ppr)*0.12.
...
// the EXACT fx_bus bleed term (fx_bus process: rin = rev + d*0.2 + (ppl+ppr)*0.12)
process(del, pp) = d*0.2 + (ppl + ppr)*0.12
with {
  d   = fbdel(del) * dgain;
