// faust/live/engine.js — browser module. Loads the PRECOMPILED WASM artifacts
// from faust/dist/ (no libfaust in the client), builds one small AudioWorkletNode
// per voice instance, and schedules faust/data/fixture.json (real buildEvents
...
// Asset paths resolve against THIS module, not the host document: engine.js lives
// in engine/faust/live/, dist/ and data/ hang off engine/faust/ one level up.
const FAUST = new URL("..", import.meta.url).href;
...
export async function createEngine(ctx, fixture) {
  ...
  for (const dsp of [...new Set(Object.values(VOICE_DSP))]) {
    factories[dsp] = await FaustWasmInstantiator.loadDSPFactory(
      `${FAUST}dist/${dsp}-module.wasm`, `${FAUST}dist/${dsp}-meta.json`);
  }
  ...
  async function dx7Load(name) {
    presets = presets || await (await fetch(FAUST + "data/dx7-presets.json")).json();
