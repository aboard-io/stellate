...
(function (root) {
  ...
  function features(state){
    ...
    // CORE KIT ONLY: the decorative perc lane (clap/rim/ride/crash/perc)
    ...
    return {
      ...
      // not fall off their own diagonals (matrix regression).
      ...
      // neoclassical deep pass: a SAMPLED PIANO lead (felt_piano) is
      ...
      // organ pass (Part B): HAMMOND counts as acoustic (organ-grade
      ...
      // (KERNEL-V4 §3.6): the acoustic membership grades now READ the
  ...
  const TARGETS = {
    ...
    neoclassical:{ bpm:[55,85,2], drumDensity:[0,.5,3], acoustic:[.8,1,3], humanize:[.28,.7,2],
                ...
                rubato:[.015,.05,2] },   // deep pass: acoustic lo .6->.8 (the voice IS a real/sampled piano now — organ pads are purged, .6-grade seeds no longer exist); drumDensity hi admits the whisper key-thunks (~.1-.3/beat, kit still off); wash re-centered on the dryer close-mic reverb; rubato REQUIRED — the one feature ambient/newage/vaporwave structurally lack
    ...
    citypop:  { bpm:[90,108,3], seventh:[.8,1,2], swing:[.03,.14,2], wash:[0,.2,2], crackle:[0,.12,2],
                motion:[.5,1,1], acoustic:[0,.85,1], hatDensity:[.35,1.4,1], pump:[0,.12,1], comp:[.05,.3,1] },   // bright maj7 pop, DRY next to vaporwave, no disco organ, light master. boopy-fix: acoustic cap .3->.85 — half the seeds now lead on a REAL jazz-guitar/electric-grand/sax (.8) beside the DX7 e-piano gloss; the .85 cap admits them (excludes felt-piano's 1.0). The separator vs italo was never acoustic (both admit synth-0); italo unaffected (still passes citypop's widened cap as it passed .3)
    ...
    phonk:    { bpm:[122,146,3], crackle:[.3,.7,3], sub:[.6,1,2], softTop:[1,1,2], swing:[0,.12,1],
                hatDensity:[.5,1.5,1], motion:[0,.7,1], comp:[.3,.7,1] },   // tape filth at dubstep tempo — dubstep is CLEAN, lofi is SLOW; kit floors track the real (sparse) hat density, recalibration after the lerp batch shifted z-scoring
    ...
    coldwave: { bpm:[96,122,3], wash:[0,.15,3], crackle:[.1,.4,2], seventh:[0,.4,2], swing:[0,.07,2],
                snareBalance:[.2,1,2], pump:[0,.15,1], leadVoices:[1,3,1], acoustic:[0,.2,1] },   // dry triads + cassette hiss — italo sparkles, this shrugs. deep pass: coldwave is BASS-FORWARD post-punk — the drum machine sits BEHIND the lead bass, snare recessed (renders snareBalance .30-.45). Floor .4->.25 + weight 1->2 makes the recessed snare a real signature: chinawave's whole claim is the "march snare UP" ([.55,1.5], renders .74-.90), so this is the structural break of the coldwave==chinawave margin-0 tie (chinawave was scoring 100 off coldwave's tracks — the two shared 110bpm/dry/triad/crackle and only the snare mix separates them)
    ...
    // ======== GENRE-EXPANSION targets (fences designed against measured renders; see genre-kernel) ========
    ...
    /* /genre-tool:strawdub:targets */
    /* genre-tool:wickershimmy:targets */
    wickershimmy:{ bpm:[110,131,3], chopUse:[0.4,0.6,3], acoustic:[0.656,1,3], breakUse:[0,0.08,2], drumDensity:[0.85,1.86,2], wash:[0,0.234,1], swing:[0,0.081,1] },
    /* /genre-tool:wickershimmy:targets */
    ...
    /* /genre-tool:seraphswing:targets */
    /* genre-tool:androidlament:targets */
    androidlament:{ bpm:[51,78,3], drumDensity:[0,0.543,3], wash:[0.399,0.671,2], rubato:[0.01,0.039,2], pump:[0,0.1,1], motion:[0.487,1,1], humanize:[0.04,0.369,1], acoustic:[0,0.12,3] },
    /* /genre-tool:androidlament:targets */
    /* genre-tool:lasertemple:targets */
    lasertemple:{ bpm:[102,123,3], hatDensity:[0.3,2.18,2], drumDensity:[1.118,3.142,2], wash:[0.142,0.458,1], pump:[0.002,0.3,1], motion:[0.487,1,1], acoustic:[0,0.12,3], bedUse:[0.71,0.95,3] },
    /* /genre-tool:lasertemple:targets */
    /* genre-tool:oscillatorminuet:targets */
    oscillatorminuet:{ bpm:[78,99,3], motion:[0.487,1,3], drumDensity:[0,1.885,2], wash:[0,0.226,1], humanize:[0,0.232,1], pump:[0,0.1,1], sub:[0.05,0.35,3] },
    /* /genre-tool:oscillatorminuet:targets */
    /* genre-tool:cometwhistle:targets */
    cometwhistle:{ bpm:[66,89,3], drumDensity:[0,0.95,2], wash:[0.294,0.588,2], rubato:[0.005,0.035,2], seventh:[0.85,1,2], humanize:[0.012,0.323,1], leadVoices:[1,4,1] },
    /* /genre-tool:cometwhistle:targets */
    /* genre-tool:chromepiston:targets */
    chromepiston:{ bpm:[112,133,3], pump:[0.353,0.643,3], hatDensity:[0.258,1.202,2], drumDensity:[1.154,2.266,1], swing:[0,0.071,1], crackle:[0,0.256,1], chopUse:[0,0.1,3], sub:[0.05,0.35,3] },
    /* /genre-tool:chromepiston:targets */
    /* genre-tool:patchcordmirage:targets */
    patchcordmirage:{ bpm:[88,109,3], motion:[0,0.887,2], hatDensity:[0.213,1.817,1], wash:[0.167,0.43,1], pump:[0,0.181,1], drumDensity:[1.077,3.113,1], acoustic:[0,0.12,3] },
    /* /genre-tool:patchcordmirage:targets */
    /* genre-tool:velourregatta:targets */
    velourregatta:{ bpm:[94,115,3], swing:[0,0.131,2], snareBalance:[0.31,1.07,2], drumDensity:[0.665,1.765,1], pump:[0,0.317,1], wash:[0.137,0.48,1], seventh:[0.575,1,3], softTop:[0,0,3], leadVoices:[1,4,3], sub:[0.05,0.35,3], comp:[0.216,0.663,3] },
    /* /genre-tool:velourregatta:targets */
    /* genre-tool:sorcerercape:targets */
    sorcerercape:{ bpm:[132,155,3], motion:[0.113,1,2], drumDensity:[1.104,2.696,2], hatDensity:[0.192,1.928,1], wash:[0.063,0.311,1], crackle:[0.02,0.358,1], bedUse:[0.71,0.95,3], seventh:[0.85,1,3] },
    /* /genre-tool:sorcerercape:targets */
    /* genre-tool:wizardcape:targets */
    wizardcape:{ bpm:[114,137,3], drumDensity:[1.115,2.215,2], motion:[0.487,1,2], seventh:[0.443,1,1], swing:[0,0.091,1], wash:[0.019,0.316,1], comp:[0.137,0.487,1], humanize:[0,0.319,1], sub:[0.05,0.35,2], bedUse:[0.71,0.95,2] },
    /* /genre-tool:wizardcape:targets */
    /* genre-tool:meadowmellotron:targets */
    meadowmellotron:{ bpm:[75,102,3], acoustic:[0.46,0.94,2], drumDensity:[0.022,2.478,2], rubato:[0,0.02,2], motion:[0.85,1,1], wash:[0.073,0.417,1], bedUse:[0.71,0.95,3] },
    /* /genre-tool:meadowmellotron:targets */
    /* genre-tool:hexagonstampede:targets */
    hexagonstampede:{ bpm:[124,143,3], drumDensity:[0.801,1.949,2], motion:[0.487,1,2], seventh:[0,1,2], acoustic:[0,0.78,2], comp:[0.177,0.623,1], snareBalance:[0.104,0.696,2], pump:[0,0.189,3], crackle:[0,0.197,3] },
    /* /genre-tool:hexagonstampede:targets */
    /* genre-tool:crimsoncourt:targets */
    crimsoncourt:{ bpm:[99,126,3], drumDensity:[0.104,1.096,2], variation:[0.88,1,2], wash:[0.008,0.252,1], motion:[0.487,1,1], sub:[0.41,1,1], bedUse:[0.88,1,2] },
    /* /genre-tool:crimsoncourt:targets */
    /* genre-tool:moonlagoon:targets */
    moonlagoon:{ bpm:[59,86,3], wash:[0.196,0.516,2], drumDensity:[0,1.076,2], motion:[0,0.887,1], swing:[0,0.091,1], acoustic:[0,1,1], variation:[0.88,1,3] },
    /* /genre-tool:moonlagoon:targets */
    /* genre-tool:sliderule:targets */
    sliderule:{ bpm:[133,160,3], drumDensity:[1.096,2.184,2], sub:[0,1,2], motion:[0.52,0.82,1], swing:[0,0.071,1], comp:[0.266,0.713,1], crackle:[0,0.182,2] },
    /* /genre-tool:sliderule:targets */
    /* genre-tool:crumpetwhirl:targets */
    crumpetwhirl:{ bpm:[89,116,3], swing:[0.132,0.326,2], seventh:[0.85,1,2], drumDensity:[0.73,2.13,2], acoustic:[0.46,0.94,1], motion:[0.487,1,1], offgrid:[0.311,0.535,3] },
    /* /genre-tool:crumpetwhirl:targets */
    /* genre-tool:polygonforge:targets */
    polygonforge:{ bpm:[147,174,3], drumDensity:[0.848,2.632,2], seventh:[0.3,1,2], variation:[0.858,1,2], comp:[0.418,0.924,1], sub:[0.41,1,1], crackle:[0,0.155,2], breakUse:[0,0.08,3], pump:[0,0.301,3], chopUse:[0,0.1,3] },
    /* /genre-tool:polygonforge:targets */
    /* genre-tool:moptoprattle:targets */
    moptoprattle:{ bpm:[137,162,3], drumDensity:[0.85,2.34,2], wash:[0,0.196,1], pump:[0,0.1,1], swing:[0.005,0.144,2], seventh:[0,1,1], acoustic:[0.48,0.92,2], crackle:[0.203,0.584,1], sub:[0.05,1,1], motion:[0.52,1,1], bedUse:[0.28,0.75,1], softTop:[1,1,3] },
    /* /genre-tool:moptoprattle:targets */
    /* genre-tool:meadowjangle:targets */
    meadowjangle:{ bpm:[107,130,3], drumDensity:[0.85,1.84,2], wash:[0,0.201,1], pump:[0,0.1,1], swing:[0.036,0.18,2], seventh:[0,1,1], acoustic:[0.48,0.92,2], crackle:[0.052,0.37,3], sub:[0.05,0.35,1], motion:[0.52,1,1], bedUse:[0.51,0.75,3], humanize:[0.081,0.424,2], comp:[0,0.308,3] },
    /* /genre-tool:meadowjangle:targets */
    /* genre-tool:strawberryfog:targets */
    strawberryfog:{ bpm:[71,96,3], drumDensity:[0,0.95,2], wash:[0.11,0.358,1], pump:[0,0.18,1], swing:[0,0.12,2], seventh:[0.85,1,1], acoustic:[0,0.92,2], crackle:[0.052,0.388,1], sub:[0.05,1,1], motion:[0,1,1], bedUse:[0.88,1,1], softTop:[1,1,3] },
    /* /genre-tool:strawberryfog:targets */
    /* genre-tool:octopusminuet:targets */
    octopusminuet:{ bpm:[85,106,3], drumDensity:[0.29,1.71,2], wash:[0.038,0.286,1], pump:[0,0.1,1], swing:[0,0.088,2], seventh:[0,1,1], acoustic:[0,0.92,2], crackle:[0.012,0.325,1], sub:[0.05,0.35,1], motion:[0.52,1,1], bedUse:[0.51,0.75,1] },
    /* /genre-tool:octopusminuet:targets */
    /* genre-tool:walrusfuzz:targets */
    walrusfuzz:{ bpm:[126,144,3], drumDensity:[0.73,1.77,2], wash:[0,0.22,1], pump:[0,0.2,1], swing:[0,0.118,2], seventh:[0.85,1,3], acoustic:[0.48,0.92,2], crackle:[0.002,0.338,1], sub:[0.05,0.35,1], motion:[0.18,1,1], bedUse:[0.51,0.75,1] },
    /* /genre-tool:walrusfuzz:targets */
    /* genre-tool:rooftopholler:targets */
    rooftopholler:{ bpm:[95,116,3], drumDensity:[0.78,1.71,2], wash:[0,0.218,1], pump:[0,0.192,1], swing:[0.036,0.18,2], seventh:[0.85,1,1], acoustic:[0.48,0.92,2], crackle:[0.01,0.3,1], sub:[0.05,0.35,1], motion:[0.18,0.82,1], bedUse:[0.51,0.75,1] },
    /* /genre-tool:rooftopholler:targets */
    /* genre-tool:submarinelullaby:targets */
    submarinelullaby:{ bpm:[53,76,3], drumDensity:[0,0.79,2], wash:[0.134,0.409,1], pump:[0,0.1,1], swing:[0,0.088,2], seventh:[0,1,1], acoustic:[0.68,1,2], crackle:[0,0.28,1], sub:[0.05,1,1], motion:[0.85,1,1], bedUse:[0.88,1,1] },
    /* /genre-tool:submarinelullaby:targets */
    ...
    /* /genre-tool:beakstampede:targets */
    /* genre-tool:velvetconveyor:targets */
    velvetconveyor:{ bpm:[109,128,3], drumDensity:[0.74,1.81,2], wash:[0,0.262,1], pump:[0,0.1,1], swing:[0.004,0.138,2], seventh:[0,1,1], acoustic:[0.68,1,2], crackle:[0.123,0.481,1], sub:[0.05,1,1], motion:[0.85,1,1], bedUse:[0.51,0.75,1] },
    /* /genre-tool:velvetconveyor:targets */
    /* genre-tool:talcumcasino:targets */
    talcumcasino:{ bpm:[121,142,3], drumDensity:[0.86,1.86,2], wash:[0,0.256,1], pump:[0,0.195,1], swing:[0,0.118,2], seventh:[0,1,1], acoustic:[0.68,1,2], crackle:[0.223,0.581,1], sub:[0.05,0.35,1], motion:[0.85,1,1], bedUse:[0.51,0.75,1], softTop:[1,1,3] },
    /* /genre-tool:talcumcasino:targets */
    /* genre-tool:capesnap:targets */
    capesnap: { bpm:[96,114,3], drumDensity:[1.2,2.32,2], wash:[0,0.173,1], pump:[0,0.176,1], swing:[0,0.122,2], seventh:[0,1,1], acoustic:[0.68,0.92,2], crackle:[0.173,0.531,1], sub:[0.05,1,1], motion:[0.18,1,1], bedUse:[0.01,0.25,1], comp:[0.446,0.778,3] },
    /* /genre-tool:capesnap:targets */
    /* genre-tool:chromeufo:targets */
    chromeufo:{ bpm:[85,104,3], drumDensity:[1.05,3.21,2], wash:[0,0.257,1], pump:[0,0.212,1], swing:[0.046,0.19,2], seventh:[0.85,1,1], acoustic:[0.48,0.92,2], crackle:[0.011,0.34,1], sub:[0.05,1,1], motion:[0.18,1,1], bedUse:[0.63,0.87,1], softTop:[0,0,3] },
    /* /genre-tool:chromeufo:targets */
    /* genre-tool:mirrorseven:targets */
    mirrorseven:{ bpm:[91,109,3], drumDensity:[0.6,2.25,2], wash:[0,0.205,1], pump:[0,0.187,1], swing:[0.076,0.22,2], seventh:[0.85,1,1], acoustic:[0,0.92,2], crackle:[0.033,0.306,1], sub:[0.05,1,1], motion:[0.52,1,1], bedUse:[0.51,0.75,1], comp:[0.346,0.709,3] },
    /* /genre-tool:mirrorseven:targets */
    /* genre-tool:sundialsyrup:targets */
    sundialsyrup:{ bpm:[61,84,3], drumDensity:[0.35,1.88,2], wash:[0.07,0.374,1], pump:[0,0.1,1], swing:[0.057,0.213,2], seventh:[0.85,1,1], acoustic:[0,0.92,2], crackle:[0.006,0.339,1], sub:[0.05,1,1], motion:[0.52,1,1], bedUse:[0.38,0.75,1], softTop:[1,1,3] },
    /* /genre-tool:sundialsyrup:targets */
    /* genre-tool:sequinfreight:targets */
    sequinfreight:{ bpm:[101,120,3], drumDensity:[0.74,1.78,2], wash:[0.02,0.26,1], pump:[0.121,0.459,1], swing:[0.014,0.148,2], seventh:[0.85,1,1], acoustic:[0,0.92,2], crackle:[0.091,0.355,1], sub:[0.05,0.35,1], motion:[0.18,1,1], bedUse:[0.01,0.25,1], softTop:[1,1,3] },
    /* /genre-tool:sequinfreight:targets */
    /* genre-tool:rollerlacquer:targets */
    rollerlacquer:{ bpm:[105,124,3], drumDensity:[0.64,2.38,2], wash:[0,0.211,1], pump:[0.201,0.537,1], swing:[0.035,0.174,2], seventh:[0.85,1,1], acoustic:[0,0.12,2], crackle:[0,0.174,1], sub:[0.05,0.75,1], motion:[0.18,1,1], bedUse:[0.51,0.75,1] },
    /* /genre-tool:rollerlacquer:targets */
    /* genre-tool:longshipwhip:targets */
    longshipwhip:{ bpm:[170,196,3], drumDensity:[1,2.6,2], acoustic:[0.68,0.92,2], pump:[0,0.167,2], crackle:[0,0.1,1], comp:[0.481,0.937,1], swing:[0,0.063,1], bedUse:[0.51,0.75,1] },
    /* /genre-tool:longshipwhip:targets */
    /* genre-tool:bogironwallow:targets */
    bogironwallow:{ bpm:[42,63,3], drumDensity:[0.17,1.57,2], wash:[0.134,0.447,2], acoustic:[0,0.72,1], crackle:[0,0.267,1], sub:[0.85,1,1], comp:[0.35,0.704,1] },
    /* /genre-tool:bogironwallow:targets */
    /* genre-tool:barrowwake:targets */
    barrowwake:{ bpm:[35,54,3], drumDensity:[0,0.69,2], acoustic:[0.48,0.72,2], wash:[0.412,0.731,1], crackle:[0,0.183,1], motion:[0,1,1] },
    /* /genre-tool:barrowwake:targets */
    /* genre-tool:ravensquall:targets */
    ravensquall:{ bpm:[155,180,3], hatDensity:[0.26,1.96,2], drumDensity:[1.16,3.18,2], crackle:[0.067,0.385,2], wash:[0.208,0.534,1], pump:[0,0.157,1], acoustic:[0,0.12,1], breakUse:[0,0.08,3] },
    /* /genre-tool:ravensquall:targets */
    /* genre-tool:runeromp:targets */
    runeromp: { bpm:[96,123,3], drumDensity:[0.75,2.52,2], acoustic:[0,0.92,2], swing:[0,0.108,2], wash:[0.071,0.33,1], sub:[0.45,1,1], comp:[0.349,0.73,3], bedUse:[0.51,0.75,3], pump:[0,0.198,3] },
    /* /genre-tool:runeromp:targets */
    /* genre-tool:meadhallbellow:targets */
    meadhallbellow:{ bpm:[86,107,3], drumDensity:[0.68,3.28,2], bedUse:[0.71,0.95,2], snareBalance:[0.04,1.34,1], wash:[0.154,0.493,1], acoustic:[0,0.92,1], comp:[0.285,0.74,3], humanize:[0.001,0.308,3] },   // bpm FLOOR 81->86 (the anchor's own authored floor): the measured fence carried 5 bpm of slack below anything this genre plays, and the slow/sub/bed-heavy dub camped in it — dub renders 69-82, so the floor alone fences the whole pair apart. MEASURED: meadhallbellow's own bpm is 87-101 over 12 seeds, self unchanged
    /* /genre-tool:meadhallbellow:targets */
    /* genre-tool:valkyrieswoop:targets */
    valkyrieswoop:{ bpm:[139,162,3], drumDensity:[1.11,2.83,2], acoustic:[0,0.92,2], wash:[0.131,0.38,2], comp:[0.459,0.896,1], crackle:[0,0.1,1], swing:[0,0.069,2], pump:[0,0.2,3], leadVoices:[1,3,3], sub:[0.05,0.35,3] },
    /* /genre-tool:valkyrieswoop:targets */
  ...
  if(isNode && require.main===module){
    ...
    if(cmd==="matrix"){
      // matrix speed round: features are computed ONCE per
