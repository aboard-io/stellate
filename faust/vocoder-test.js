...
async function main() {
  ...
  const code = fs.readFileSync(path.join(__dirname, "dist", "robot_choir-module.wasm"));
  const factory = { cfactory: 0, code: new Uint8Array(code), module: await WebAssembly.compile(code),
    json: fs.readFileSync(path.join(__dirname, "dist", "robot_choir-meta.json"), "utf8"), poly: false };
