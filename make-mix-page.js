...
const SE = require("./strudel-export.js");   // per-track "⧉ strudel" links
...
const rows = tracks.map(t => {
  ...
  // remix link: the track's state exported as Strudel code, into our self-hosted REPL
  let strudel = "";
  const stPath = path.join(dir, `track-${nn}.state.json`);
  if (fs.existsSync(stPath)) {
    try {
      const st = JSON.parse(fs.readFileSync(stPath, "utf8"));
      strudel = ` · <a href="../remix.html#${SE.codeHash(SE.toStrudel(st))}">⧉ strudel</a>`;
    } catch (e) { /* unexportable state: skip the link */ }
  }
  ...
  return `    <div class="trk">
      ...
      <div class="meta">
        ...
        <div class="coords">${t.bpm} bpm · ${KEYS[t.key]} · ${mmss(t.seconds)} · ${t.meta.form} form${vid}${strudel}<br>
