...
| **3** | **3a** a PROBE agent drives the deployed staging page as a user through the whole story (blank state → seed → pick a genre → Rules → build a band → structure grid → mix → produce → screensaver), forbidden to fix, reports defects with evidence | — | **DONE** — `scratch/maps-2026-09-01/probe-report.md`: 26 ranked defects with evidence, driven at 390x844 and 1280x900 on 152c6cd / sw v249, zero pageerrors, deploy integrity checked file by file. Four HIGH: the nav list drawing under the pinned foot, the `bars` rule reaching nothing, the share link losing every edit, and the missing `duplicate` act. |
| **3** | **3b** the fix round from 3a's report; deploy | — | **IN PROGRESS** — two agents, disjoint file sets, one working tree. A: the shell (`ui/eight.js`, `ui/atlas.js`, `nu.css`, the shell gates). B: the data tier and the panels' own modules (`rules.js`, `avail.js`, `fields.js`, `compose.js`, `song.js`, `document.js`, `precompose.js`, `ui/rules.js`, `ui/selects.js`, `ui/preview.js`, `audio/*`, and their gates). Deploy after both. |
...
**WHAT WAVE 3b REGISTERED, AND IT IS ALL REWRITES IN PLACE** (2026-09-02). The
fix round added no new gate FILE — a defect the probe found on the rendered
page is a defect an existing gate should have caught, so the honest response is
to make that gate ask a better question rather than to stand a new one beside
it. Every one below keeps the sentence it replaces, above the new one, with the
finding quoted:

| gate | what it now asks, and why it did not before |
|---|---|
| `rules` R6 | **The tier of every editable rule is MEASURED, not read.** The old R6 asserted three hand-picked fields in each direction, which is an example and not a law, and five rows were lying under it: the probe caught `bars` ("rederive 'render' … reaches nothing"), and the sweep that answered it caught `artic`, `maxHold`, `fx` (all compose, all claiming render), `phrase` (render, claiming compose) and `instr` (claiming compose and reaching NOTHING — `instruments.js instrOf` read the CATALOGUE row past the hand's own edits). R6 now sweeps every rule's own legal values over ten anchors and asks the two questions the three tiers are made of. Plus **R6b**: a said-only rule says its reason on every anchor and refuses the write. |
| `rules-view` R4, R8 | R4 drove `maxHold` as its render-tier example and now drives `phrase`, because `maxHold` measured as a compose. **R8 is new**: the tier SENTENCE is said once per axis block plus once on any row that departs from it — the probe counted twelve copies of "the record is written again at this seed" in one panel. Every row still declares its tier as `data-tier`, which costs no words. |
| `pace-meter` A1 | The paced roster is READ off the catalogue instead of typed into the gate. It held a literal three names against `GENRES`' own four (the corpus round shipped `skapunk`), so the gate went red for a row doing exactly what the gate exists to permit. The claim — ABSENT IS TODAY AT THE DEAL, no anchor composes a word it did not declare — survives whole and now prints its roster. |
| `precompose` G1 | The bpm fence is `fields.js BPM_LO/BPM_HI` and not a third literal. G1 allowed 60..200 while compose threw outside 70..160 and song.js's door dropped anything outside 70..160 — three fences, so a change could land without one of the three noticing. All three are 40..220 now (the walls-down law; `poppunk` takes the 167 its own corpus note measured and could not spend). |
| `commute` | `playLabel` read the transport button's whole `textContent` and compared it to `"play"`. Wave 1a gave the button a FACE — glyph plus a `.nu-vh` word — so the text is `"▶play"`, the comparison failed on every call, and `press()` took its else branch: click (start), 400 ms, click (STOP), then sixty seconds of silence reported as "sound arrives with the wire cut: no sound in 60s" on a page that was playing. It reads the `.nu-vh` word now. `domLine` read `#engine`, an element index.html deleted on 2026-08-28 when the readout moved into the logger; it reads `#nu-log`, which is stricter (the log keeps its predecessors). |
