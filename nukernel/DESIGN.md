...
3. **Special row** — the RECORD's seven SPECIAL ROWS (RULES · TIME · CHORDS · MOTIFS · MASTER · PRODUCE · PERFORMANCE), which are the sections of component 18 since 2026-09-06 (TABLE.md §14) and which are seven of the record's EIGHT SURFACES — component 18 owns that count and says what the eighth is — and MIX, which is not one of them because a fader is the player's: a merged row, ONE LINE at rest and `--tap` tall (the word left, the sentence or the count right, ellipsised and never wrapped, a hairline under, no plate and no tint, no chips or lozenges inline, and a lamp inside that line or not at all); expanded = its sheet; keeps its open state across a recompile. It does NOT pin at rest — it scrolls out of the way — **and since 2026-09-08 it does not pin at all** (TABLE.md §22): its sheet is a card over the record, so there is nothing under it to be the header of, and the card carries the row's own word instead. *(It read* **"pins only as the HEADER of its own open sheet, at the pane's top edge, carrying the × at its right end (TABLE.md §13a)"** *while a sheet was a row. The one-pin guarantee §13 was written for is now structural rather than arithmetic — the sheet is not in the scrollport at all — and `stick()` is one branch shorter for it.)*
4. **Sheet** — a vector as rows (label · value · clear-back), wrapping, opened as a **CARD** over the record: a scrim and a plate, `role="dialog"` with `aria-modal`, capped at 88% of the space between the two fixed bands so a strip of scrim is always aimable, centred and 44rem wide above 700px, the card its own and the arrangement's ONLY scroller. Its HEADER is the sheet's name at the start and the × at the end — and on a cell sheet the name IS component 5, so the address and the four verbs are the title bar. Dismissed four ways and by nothing else: the ×, Escape, a press on the scrim, and the owner's closer. Focus lands on the card (which reads its name first), Tab is held inside it, and on close focus goes back to the control that opened it — or to the pane, when an op inside the card replaced that control. *(It read* **"full pane width, in flow (never a modal), opened as the next `<tr>` under the row that owns it; its owner row is its header and is the pane's ONE pin while it is open"** *until 2026-09-08 — TABLE.md §22, Paul:* **"Instead of expanding sections in the editor and inserting them below the selected point just make them modals with easy dismissal."** *The in-flow law was right about the disease and wrong about the cure: an eighteen-field cell vector opened a `<tr>`* ***779.4px tall*** *at 390×844 — the whole glass — inserted between the row you tapped and every row below it, so it bought "you can still see the cell" and paid with "you can no longer see the record". At that size the accordion WAS a full-screen surface, lying about its shape. What the old law protected is kept: the card prints the owner's name at its top edge, so which row you are editing is on the glass as a WORD rather than as a position, and the distance between the mark you pressed and the editor it opened is zero at every width. The one law that BENDS is* **"menus never scroll inside themselves"** *(2026-08-16): vertical space is abundant in a document and is exactly 844px on a phone, so the card scrolls — and it is the one thing that does, which is why the record behind it now holds still where the accordion shoved it. `ui/wordgrid.js`'s grids — the mix board's `trimgrid` — still open the old accordion row; §22 was about the editor and does not sweep a surface it was not about.)*
5. **Cell sheet header** — the address of the selection, then undo · redo · copy · paste. Present only while a cell sheet is open. **It is the card's TITLE BAR since 2026-09-08** (TABLE.md §22): a cell's sheet arrives with a head already, and its address is the same sentence a title would print — drawn in both places the card said `bass 1 × stab` twice, four pixels apart. So the head IS the title, which puts the four verbs OUTSIDE the scroller (undo is on the glass at every scroll position of an eighteen-field vector, which it never was in the accordion) and keeps `data-k="taddr"` at the home `test/table-inventory.json` files it under. *(It was in flow at the top of the open row, never fixed and never sticky, until §22; and before §13a.6 it was the* **formula bar**, *a strip above the pane that became a bottom sheet on a phone — 105.8px of an 844px screen at rest, measured.)* Undo and redo therefore still live where the change was made. (It was the **formula bar**, a strip above the pane that became a bottom sheet on a phone — 105.8px of a 844px screen at rest, measured — until TABLE.md §13a.6 moved its head into the sheet and found its readout was the sheet's own first group all along. Undo and redo therefore live where the change was made.)
...
25. **Index** (`<nu-index>`, and the app's own `#atlasIndex` — TABLE.md §23,
    2026-09-08, Paul: *"The genre list should be 100% wide with no left or
    right borders. The 'Find a genre' should be integrated into it and the
    'clear text' button is now on the right with no label, but it should be an
    (x) on the inside of the button. These should be in the design system."*)
    — a searchable list of records: a year, a name, a place, and one row you
    are on. **ONE BOX WITH A FIELD ACROSS ITS TOP**, and every clause of that
    is a decision:
    **IT IS THE FULL WIDTH OF THE GLASS.** Full bleed the way `.nu-bar` does
    it — cancel the panel's `--s4` gutter with a negative margin and let the
    CONTENTS re-pay it, never `100vw` (vw includes the scrollbar gutter, and a
    sideways scroll on the body is the one thing this sheet exists to
    prevent). Measured: 364.4px at x 12.8 → **390 at x 0** on a 390 phone.
    **NO SIDE BORDERS AND NO CORNER** — a rule down the left of a box whose
    left edge IS the screen is a line drawn 0px from nothing, and a corner is a
    thing a box has. What is kept is the rule ABOVE and BELOW, which is what
    still says where the catalogue starts and stops in a scrolling column.
    **THE SEARCH IS THE LIST'S HEAD, PINNED, not a strip above it.** Two plates
    with a gap between them read as a filter and a list; one plate with a field
    across its top reads as a list you can type into. It is `sticky` because
    the box scrolls 502 rows and a search that leaves the screen at row nine is
    a search you have to scroll back to; it is the only pinned thing inside the
    box, so §13's one-band law is not in question. Its inline padding is the
    rows' own `--s4`, so the field's left edge stands on the same line as every
    year under it.
    **THE CLEAR IS AN × INSIDE THE FIELD.** Absolutely placed at the field's
    inline end, drawn over the text, `--tap` square as a HIT AREA with no plate
    of its own — a border and a ground around a mark that stands inside another
    control's border draws a box inside a box. What says it is pressable is the
    mark going from `--dim` to `--ink` under a hand. It is `hidden` until there
    is something to clear (a control always present and inert half the time is
    the silent grey §3 legislates against), and the mark is **`\u00d7` and never
    `\u2715`** — U+00D7 is Latin-1 and renders in every font this page can be
    given, which is `ui/glyph.js`'s own argument about this exact sign.
    *(THE APP'S × HAD NEVER RENDERED AT ALL, which is the "no label" in Paul's
    sentence: `ui/atlas.js` built it as `el("span", "\u2715")` and `el` there is
    `(t, a) => Object.assign(document.createElement(t), a || {})` — its second
    argument is a property bag, so `Object.assign(span, "✕")` assigned the
    string's indexed characters onto the element and returned an EMPTY span.
    A blank square had been shipping since the day the control landed.)*
    The element and the app say all four of these things in the same order in
    the same stylesheet, so the day either moves the other is one grep away.

26. **Motif field** (TABLE.md §24, 2026-09-08, Paul: *"The motif selector should be visual and let me see motifs and make new motifs and assign multiple motifs and click to edit motifs."*) — the one field whose options are **pictures**, and the only one that mints and navigates. A wrapping grid of `9ch` cards, never a track: §24 has just taken the sideways axis off this sheet and a field that reintroduced one would be the same complaint a level down. Each card is `ui/preview.js`'s own `<svg>` of the actual phrase — one bar per step, height by velocity — with the name under it and the provenance under that, **so twelve motifs read as twelve SHAPES before a single name is read**, which is the sentence that file was written under. The last card is the `+`, which calls the bank's own `addCell` (free-name rule included) and **assigns what it made**, because a hand that asked for a new motif here asked for it for this cell. A ✎ rides each card's corner and opens that motif in the bank's editor — **a separate target, never a second meaning for the card**, whose meaning is ASSIGN; a control that sometimes writes and sometimes navigates is what this page keeps deleting. The absent detent keeps its place as a card with no picture, because "inherit" is a real choice. **A CARD IS A PLATE AND THAT IS DELIBERATE** — a border is what this page draws around a TARGET, and these are sixteen targets; it is named here so component 12's plate census can tell a declared plate from a stray one. *(Assigning SEVERAL motifs is NOT built: `material.cell` holds one name, so whether a cell plays several in sequence across its bars or layered at once is a question about the MODEL and not a widget. Asked, not assumed.)*
...
- **THE TOP IS WHERE YOU GO, THE BOTTOM IS WHAT YOU HEAR** (TABLE.md §16, 2026-09-06): every navigation and every arrangement control is behind ONE hamburger at the top **left** (2026-09-07 — *"Sorry hamburger should be on left"*; it was the top right from 2026-09-06 until then, and TABLE.md §20 owns the reversal); every transport control is in the bar at the foot **plus the TAPE, which is a readout and not a control** (2026-09-08, TABLE.md §22, Paul: *"Move the playback bar to the bottom bar and expand the genre as header but don't put a border around it"* — the tape rode the top strip from 2026-09-06 under a sharpened wording of this law, *"the strip is identity, status and navigation"*, and the sharpening was bought with a measurement that has since changed: the strip's dead gap is the record's NAME now and the bar has room, so a tape stands on the band the ▶ that moves it is on); nothing else is chrome. **AND THE RECORD'S NAME IS A HEADER, NOT A PLATE** — no border, no ground, no shadow: a border is what this page draws around a TARGET and a header is a thing you READ, so the strip is one surface with a name across it. The box has exactly two modalities — a VIEW (chosen from that list) and a SHEET (**a card over the table** since 2026-09-08, component 4) — where it had five. **AND THERE IS NO ROOM FADER UNDER A THUMB** (§22, Paul: *"Just get rid of the volume control on mobile if it can't work. Leave it on desktop."*): a phone gets the WAV-first engine route, which has no live output graph, so the level rides `mvol` into the next bar fed to the worker and lands seconds later when the queued segments have played out — a fader you drag and hear nothing from is a control lying about being one. `(pointer: coarse)` hides it in the bar (the page's own definition of a thumb, `src/menus/pick.ts coarse()`); the store, `#vol` and the board's `#vol2` are untouched, and a phone's hardware volume is the control that was always going to be reached for anyway.
