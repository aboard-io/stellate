...
const ASK = {
  ...
  radiohead:  { q: "Radiohead", why: "Oxford 1997 is OK Computer, the middle of the span Paul named (\"creep to kid a and esp karma police\"). The BAND is the subject because the row spans records rather than being one: its ambient parent is Kid A's side and its postpunk parent is the debut's, and no single album article carries both. The article's own origin is Abingdon, Oxfordshire — the label says Oxford because Abingdon measures 3 km from Sutton Courtenay on this map and would have worn Slowdive's village. Not OK Computer, which would anchor the row on one release against the `velvets` ruling.", kind: "artist" },
  stoneroses: { q: "The Stone Roses", why: "Manchester 1988 is the debut, whose own infobox says \"Recorded June 1988 - February 1989\" — 1988 is when the record was made, and Manchester|1989 was already bleeptechno's key. The band is the subject because the row's whole argument is that it is EARLIER than the scene it made: `madchester` is Manchester 1990 here, so this is the ancestor and the edge runs forward, which is the maringa rule. Not The Stone Roses (album), same ruling as radiohead.", kind: "artist" },
  wings:      { q: "Wings (band)", why: "Lagos 1973 is Band on the Run, cut in Lagos, which is the record's own place and frees London|1973 for spacerock. The row exists because `beatles` (Liverpool 1962) has no CHILD in this table — every other descendant is a scene, and this is the one act that is the same writer carrying on, which is why that edge carries 0.6. The disambiguated title is the band; the bare word is the anatomical article.", kind: "artist" },
  skinnypuppy:{ q: "Skinny Puppy", why: "Vancouver 1986 is Mind: The Perpetual Intercourse, and the article's origin is Vancouver, British Columbia. The row's load-bearing fact is a DATE REFUSAL: `ebm` is the obvious parent and is wrong, because ebm is Chicago 1989 in this table — three years AFTER this record — so the parents are what was actually available to it, waxtrax, postpunk and kraftwerk.", kind: "artist" },
  ministrysynth: { q: "With Sympathy", why: "Chicago 1983, and the ALBUM is the subject here rather than the band, which is the opposite of every other row in this block and is the point. Paul asked for \"all the eras of ministry\": the later ones are already in the table as `industrialmetal` (Chicago 1988, which names Ministry in its own comment) and `waxtrax`. What was missing is the synth-pop debut the band spent its career disowning — a different genre, so a different row, and the album article is the only subject that is THAT era and not the whole act.", kind: "record" },
  katebush:   { q: "Kate Bush", why: "Kent 1985 is Hounds of Love (16 September 1985 per the album's infobox); the place is Bexleyheath in the historic county of Kent, her own origin, and London|1985 was already synthduo's key. The artist is the subject because the row is built on a career fact rather than one record's: a pop singer writing a suite for side two. This table has no art-rock rung to point at and the row says so in `wants` rather than borrowing one.", kind: "artist" },
  petergabriel: { q: "Peter Gabriel", why: "London 1977 is the solo debut, not So (1986) — \"all the eras\" starts where the solo career does, and London|1986 was already sophistirock's key. The artist is the subject because the Genesis era is a BAND and a different row's job; this table has no prog anchor to hang it on, which the row declares in `wants` instead of fudging.", kind: "artist" },
  fairuz:     { q: "Fairuz", why: "Beirut 1957, the Baalbeck years. Her article gives the genres as Arabic music, Lebanese music and TARAB and years active from 1950 — so the `tarab` parent (Cairo 1934) is named by the subject itself rather than inferred by me, which is the strongest kind of edge this table has. Beirut is new geography, 192 km from the nearest existing dot.", kind: "artist" },
  bronskibeat:{ q: "Bronski Beat", why: "Brixton 1984 is The Age of Consent, and BRIXTON is what the article says — I wrote London twice and moved the YEAR twice before noticing the place was the wrong part. Brixton is declared within London in atlas.js, the Muswell Hill case exactly. The band is the subject: the row's fact is a falsetto over a machine-built four-on-the-floor, which is a group's sound and not one release's.", kind: "artist" },
  omd:        { q: "Orchestral Manoeuvres in the Dark", why: "Liverpool 1980 is Organisation; the article's origin is Meols, Merseyside, which measures 11 km from Liverpool's dot and is therefore inside it. The row is the rung this catalogue lacked between `kraftwerk` (Düsseldorf 1977) and `synthpop` (Basildon 1981): a band who took the German machine and wrote pop songs on it a year before Basildon did, which is why kraftwerk carries 0.45.", kind: "artist" },
  slowdive:   { q: "Slowdive", why: "Sutton Courtenay 1993 is Souvlaki, cut at the Courtyard studio the album's infobox names. The band's article is the subject and it carries the row's whole argument in one sentence: \"debut album Just for a Day (1991) and follow up Souvlaki (1993), making the band one of the leading figures of the UK shoegaze scene\". Not Shoegaze, which is `shoegaze`'s link and is anchored on Loveless — the point of this row is that Souvlaki is the OTHER record. Not Souvlaki (album) either: the row is the band's spacious half, not one release, which is the `velvets`-not-the-banana-album ruling. The bare title is the band; the article's own hatnote separates the eponymous album, the EP and the Siouxsie and the Banshees song.", kind: "artist" },
  galaxie500: { q: "Galaxie 500", why: "Boston 1989 is On Fire, which this band's article calls \"the band's defining moment\". The band is the subject because the row is the first slowcore anchor in the table and the band, not the album, is what the Slowcore article credits: \"the band is frequently cited as one of slowcore's leading antecedents\". Not Slowcore, the genre article, which would be the wrong shape — this row is named for the act, the `velvets` and `sisters` precedent. Not the Ford car; the article's own hatnote points at Galaxie 500 (disambiguation).", kind: "artist" },
