#!/usr/bin/env node
/* test/_odear.cjs — THE EAR, WITH THE OVERDRIVE ROW SWITCHABLE IN FLIGHT.
   nukernel/export/_livetap.cjs's tap, verbatim below the flags, because that
   file is the honest instrument for a whole-mix question and this round must
   not edit it: _satdrive/pressFloat read the RING, and every stage of the
   master bus (the glue compressor, the MAKE-UP RIDER, the two lowpasses, the
   brickwall) sits downstream of it. A cut a normaliser undoes is not a cut,
   and the ring cannot tell.

   The one thing added is `--od`, the same A/B door test/_odpress.cjs carries:
   `--od off` serves audio/to-engine.js with ID_ROUTE.overdrive_guitar
