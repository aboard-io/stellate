#!/usr/bin/env node
// embed-audio-run.js — THE EMBED IS AUDIBLE. The gate that decides whether the
// embed ships at all.
//
...
// `SharedArrayBuffer` is undefined and engine/faust/live.js throws
//   "FaustLive: SharedArrayBuffer unavailable (page must be cross-origin isolated…)"
// …and the embed is a silent black box. app/live.js's NO-ISOLATION FALLBACK
...
//   2. app/live.js chose the WAV route automatically (window.__AUDIOROUTE.wavOut)
...
const RMS_FLOOR = 0.0008;          // the same threshold app/live.js calls "real sound"
...
(async () => {
  ...
  if (!(iso.route && iso.route.wavOut === true))
    fails.push("app/live.js did not auto-select the WAV route (__AUDIOROUTE=" + JSON.stringify(iso.route) + ")");
  // 3. no autoplay
