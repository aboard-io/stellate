#!/usr/bin/env node
// test/feed-links-run.js — THE FEED'S ONE PROMISE, GATED.
//
//   node test/feed-links-run.js
//
// Every item in the release feed (tools/gen-feed.js) links to a mix that shows
// off what changed. That promise is only worth making if the links actually
// LAND: a `?path=` built from app/world.js POS must restore verbatim and put
...
async function main() {
  ...
  execFileSync("node", [path.join(ROOT, "tools", "gen-feed.js"), "--historic", "--out", tmp],
  ...
  const world = fs.readFileSync(path.join(ROOT, "app", "world.js"), "utf8");
  let bad = 0, seen = 0;
  ...
  // when app/world.js POS is re-baked. Assert every deep link still starts on
  // its own star; `node tools/gen-feed.js --sitemap` is the fix.
  ...
  for (const m of sm.matchAll(/\?genre=([a-z0-9_-]+)&amp;seed=\d+&amp;path=([\d.,-]+)/g)) {
    ...
    if (Math.hypot(x - p[0], y - p[1]) > 40) { fail(`sitemap: ${m[1]} link is off its star — run \`node tools/gen-feed.js --sitemap\``); stale++; }
  }
