...
//
//   node test/gates/genre-specs.test.js
//
...
//
//   node tools/genre/genre-tool.js export --all      # the fix, when it fails
"use strict";
const fs = require("fs");
...
try {
  const out = execFileSync("node", [path.join(ROOT, "tools", "genre", "genre-tool.js"), "export", "--all", "--dry-run"],
  ...
  drift = changed === 0 ? `all ${genres.length} specs match the live kernel`
    : `${changed} spec(s) differ from the kernel — run: node tools/genre/genre-tool.js export --all`;
