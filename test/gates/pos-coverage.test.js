#!/usr/bin/env node
// test/gates/pos-coverage.test.js — the STAR-MAP POS COMPLETENESS gate. Plain-node, no browser.
//
// THE BUG THIS CATCHES (a production outage): a genre added to the kernel
// (GenreKernel.GENRES) but NOT to app/core/world.js POS. app/main.js boot() runs
// app/map/starmap.js computeGenreLayout(), which EARLY-RETURNS only when POS already
...
//
// It parses app/core/world.js's `export const POS={...}` STATICALLY (world.js is an ES
...
//
//   node test/gates/pos-coverage.test.js
//
// Wired into verify.sh (the "poscover" row). Zero deps; consumes engine/
// genre-kernel.js + app/core/world.js read-only, so new genres are covered on land.
"use strict";
const fs = require("fs");
const path = require("path");
const K = require("../../engine/genre-kernel.js");

const WORLD = path.join(__dirname, "..", "..", "app", "core", "world.js");
...
if (missing.length) {
  console.error(`\n  FAIL: ${missing.length} genre${missing.length === 1 ? "" : "s"} in GenreKernel.GENRES MISSING from app/core/world.js POS:`);
  for (const g of missing) console.error(`      - ${g}`);
  console.error(`\n  A missing genre drops app boot into computeGenreLayout's relaxation and`);
  console.error(`  CRASHES the renderer. Add each above to app/core/world.js POS (see`);
