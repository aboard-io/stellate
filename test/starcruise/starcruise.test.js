#!/usr/bin/env node
// test/starcruise/starcruise.test.js — headless proof for the 🛸 STAR-CRUISE 3D video mode
...
async function main() {
  ...
  // app/map/starmap.js computeGenreLayout(), which relaxes the whole genre field and,
  ...
  // app/core/state.js exports. This exercises the REAL getS()->window.__S read path the
  ...
  if (gx.err) { ok(false, "GX. galaxy probe threw :: " + gx.err); }
    ...
    // (app/map/glyphs.js) — compare de-glyphed, as GX1's label check does.
