#!/usr/bin/env node
// test/unit/dx7.test.js — offline proof that decoded cartridge presets actually drive
...
const presets = require("../../engine/faust/data/dx7-presets.json");

const SR = 44100, BS = 64, SEG = 3.0, NOTES = [[0.0, 220], [0.7, 277.18], [1.4, 329.63]]; // A3 C#4 E4

async function loadFactory(name) {
  const code = fs.readFileSync(path.join(__dirname, "..", "..", "engine", "faust", "dist", `${name}-module.wasm`));
  return { cfactory: 0, code: new Uint8Array(code), module: await WebAssembly.compile(code),
           json: fs.readFileSync(path.join(__dirname, "..", "..", "engine", "faust", "dist", `${name}-meta.json`), "utf8"), poly: false };
}
...
async function main() {
  ...
  // scratch/, never beside the test: audio is derived and gitignored (the one
  // rule), and the deploy's wav exclude is ROOT-anchored — a wav left in test/
  // is invisible to it and rides the rsync to the droplet. scratch/ is ignored
  // by both.
  const out = path.join(__dirname, "..", "..", "scratch", "dx7-render.wav");
  fs.mkdirSync(path.dirname(out), { recursive: true });
  fs.writeFileSync(out, Buffer.concat([hdr, data]));
  console.log("wrote scratch/dx7-render.wav");
}
