...
//   node test/unit/meter.test.js [--no-press]
//
// Gates, in order:
...
// ---- 4: absent-meter byte-identity vs HEAD ----
{
  ...
  try {
    ...
    for (const rel of ["engine/faust/data/dx7-presets.json", "engine/faust/dx7-presets.json"]) {
      try { presets = execFileSync("git", ["show", "HEAD:" + rel], { cwd: path.join(HERE, "..", ".."), maxBuffer: 64 * 1024 * 1024, stdio: ["ignore", "pipe", "ignore"] }); break; } catch (e) {}
    }
    if (!presets) throw new Error("dx7-presets.json not in HEAD");
...
(async () => {
  ...
  if (noPress || !faustDeps) {
    ...
    const rms = await new Promise((resolve) => {
      execFile("node", [path.join(HERE, "..", "..", "engine", "faust", "press", "press.js"), sj, wav, "--dur", "8"],
