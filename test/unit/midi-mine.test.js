#!/usr/bin/env node
// midi-mine.test.js — gates for tools/mine/mine-midi.js (the trove miner) and the
// midi-export fixes it forced.
//   node test/unit/midi-mine.test.js
//
// Gates, in order:
...
(() => {
  ...
  if (!fs.existsSync(dir)) { console.log("SKIP  keycheck: classical_piano rip not fetched (tools/fetch/fetch-midi-trove.sh — external drive; the MIDI never lives under found/, it would deploy)"); return; }
