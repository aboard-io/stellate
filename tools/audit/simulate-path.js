#!/usr/bin/env node
// tools/audit/simulate-path.js — THE PATH SIMULATOR: fully simulate a path.
...
//   2. headless live gates     — test/browser/transit-arrival.test.js etc: real engine, real
...
//     interval in app/audio/live.js ticks glideStep without audio; explorer-ui-test
...
//     tier 2's job (test/browser/transit-arrival.test.js / test/browser/explorer-ui.test.js's ride);
...
//
// USAGE
//   node tools/audit/simulate-path.js <default|path.json|genreA,genreB,…>
...
//
//   node tools/audit/simulate-path.js default --seed 43 --pace 64
"use strict";
const fs = require("fs");
const path = require("path");
const { serve, launchChromium, capturePageErrors } = require("../../test/lib/probe-harness.js");
...
async function main() {
  ...
  const setup = await page.evaluate(async ({ inp, seed, pace }) => {
    ...
    const T = await import("/app/audio/targeting.js");
    window.__SIM_T = T;
    let wps, label;
