...
//
//   node tools/corpus-db.js build <dir> [dir...] --db <path> [--limit N]
//   node tools/corpus-db.js stats   --db <path>
//   node tools/corpus-db.js keycheck --db <path> [--rip x]     detector vs embedded sigs
//   node tools/corpus-db.js melody  --db <path> [--rip x]      interval/contour report
//   node tools/corpus-db.js near    --db <path> <id|path> [-k 10]   cosine neighbours
//   node tools/corpus-db.js bench   --db <path>
//
...
// Validate on labeled corpora before trusting (test/corpus-db.test.js).
"use strict";
const fs = require("fs");
