...
#
#   tools/deploy/deploy-stellate.sh [host]     # default root@stellate.app
#
...
echo "== release feed =="
node tools/build/gen-feed.js --historic
...
if curl -sI https://stellate.app/engine/faust/data/dx7-presets.json | grep -qiE 'cache-control:.*max-age'; then
  echo "cached OK"
else
