...
getbed() { # url | ss | dur | name | extra-af(before loudnorm)
  local out="found/$4.wav"
  [ -s "$out" ] && return 0
  echo "→ bed $4"
  curl -sL --max-time 180 -o /tmp/bed.in "$1"
  ffmpeg -y -loglevel error -ss "$2" -t "$3" -i /tmp/bed.in -ac 1 -ar 44100 \
...
synbed() { # name | aevalsrc-expr | dur | extra-af(before loudnorm)
  local out="found/$1.wav"
  [ -s "$out" ] && return 0
  echo "→ synbed $1"
  ffmpeg -y -loglevel error -f lavfi -i "aevalsrc='$2':d=${3}:s=44100" \
    -ac 1 -ar 44100 -af "${4:-anull},loudnorm=I=-18:TP=-1.5" "found/$1.wav"
}
...
if [ ! -s found/hvac_hum.wav ]; then
  ...
  ffmpeg -y -loglevel error \
    ...
    -map "[o]" -ac 1 -ar 44100 found/hvac_hum.wav
fi
...
if [ ! -s found/modem_handshake.wav ]; then
  ...
  ffmpeg -y -loglevel error \
    ...
    -map "[o]" -ac 1 -ar 44100 found/modem_handshake.wav
fi
