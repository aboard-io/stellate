...
//   C. file fonts (engine/faust/fonts.json): per font, which SAMPLERS ids the
...
function auditFileFonts() {
  ...
  let manifest = [];
  try { manifest = JSON.parse(fs.readFileSync(path.join(ROOT, "engine", "faust", "fonts.json"), "utf8")); } catch (e) { console.log("  (no fonts.json)"); return; }
  for (const f of manifest) {
    if (f.key === "fluidr3" || f.synth) continue;
    let data;
    try { data = JSON.parse(fs.readFileSync(path.join(ROOT, "engine", "faust", "font-" + f.key + ".json"), "utf8")); }
...
function dx7Sweep() {
  ...
  for (const name of names) {
    ...
    let db = null;
    try {
      execFileSync("node", [path.join(ROOT, "engine", "faust", "press.js"), sj, wav], { stdio: ["ignore", "ignore", "pipe"], timeout: 120000 });
