...
/* RESTORED 2026-09-06 from 4a4d730^ (it was deleted when band.html became
   ...
   HOME the same day: it lived in scratchpad/, which is .gitignored, so the one
   tool that measures a row against its parents was not in the repo at all and
   the auto-remix round found it by archaeology. It is a TOOL — hand-run,
   nothing in the app or the engine depends on its output — so tools/ is where
   it belongs, beside tools/genres/ and tools/mine/, and `tools/remix.js`
   requires it to name what a mined row is nearest to.
   THREE EDITS AND NO OTHERS from the deleted original: the module path points
   at nukernel/ instead of at its own directory; `featuresOf` calls an optional
   EXTRA hook so the fit can be run in a wider space without a second copy of
   the NNLS; and the feature extraction is split so that a row which is not in
   the catalogue yet can be measured — `featuresOfRow(g, bpm)` is the whole of
   the old body and `featuresOf(k)` is now one line that looks the key up.
   `nearest()` is new and is three lines over `featuresOfRow`. */
const DEP = NODE
...
function featuresOf(k) {
  const v = featuresOfRow(GENRES[k], BPM[k]);
  return EXTRA ? v.concat(EXTRA(k)) : v;
}

/* THE VECTOR OF A ROW THAT MAY NOT BE IN THE TABLE (2026-09-06). Everything
   below the first line is the body `featuresOf` has always had, moved verbatim;
   the only change is that the row and its tempo ARRIVE rather than being looked
   up, so tools/remix.js can ask "what is this mined row nearest to" before
   anybody has decided whether to keep it. `featuresOf(k)` above is the same
   question asked of a key, and it is still the only caller the EXTRA hook can
   serve, because a hook keyed by genre key has nothing to say about a row that
   has no key yet. */
function featuresOfRow(g, bpmOfRow) {
  const f = {};
  f.tempo = clamp01(((bpmOfRow != null ? bpmOfRow : 112) - 70) / 90);
  f.rate = g.rate;
...
/* NEAREST — the rows a vector sits closest to, by euclidean distance in the
   feature space above, with the six function anchors left out (they have a job,
   not a history, so "nearest to `pad`" is not a claim about music). Returned
   sorted, nearest first. This is a MEASUREMENT and not a genealogy: a parent is
   something a person argues for, and this only says where in the table a record
   lands. tools/remix.js prints it and writes it into the row's note; it does
   NOT write `parents` off it, because genres-build G2 holds that a row with
   parents carries a Place Year label and a MIDI file names neither. */
function nearest(vec, n = 5) {
  const d = REAL.map(k => {
    const w = featuresOf(k);
    let s = 0;
    for (let i = 0; i < vec.length && i < w.length; i++) {
      const x = (vec[i] || 0) - (w[i] || 0); s += x * x;
    }
    return { key: k, label: GENRES[k].label, d: Math.sqrt(s) };
  });
  d.sort((a, b) => a.d - b.d || (a.key < b.key ? -1 : 1));
  return d.slice(0, n);
}

/* THE RESIDUE OF A LOOSE ROW against a chosen set of neighbours: the same NNLS
   `fitChild` runs, over vectors rather than over keys, so a mined row can print
   the number the genealogy program is about ("what the parents explain is
   heredity; the residue is the invention") without being in the table. */
function fitVec(vec, keys) {
  const A = vec.map((_, r) => keys.map(k => featuresOf(k)[r]));
  const w = nnls(A, vec);
  const resid = vec.map((x, r) => x - keys.reduce((s2, _, j) => s2 + A[r][j] * w[j], 0));
  const mean = sum(vec) / vec.length;
  const sst = sum(vec.map(x => (x - mean) * (x - mean)));
  const sse = sum(resid.map(x => x * x));
  const wsum = sum(w) || 1;
  return { keys, fitted: w.map(x => x / wsum), rawFitted: w,
           r2: sst > 0 ? Math.max(0, 1 - sse / sst) : 0,
           residRms: Math.sqrt(sse / vec.length) };
}
...
const api = { FEATURES, REAL, featuresOf, featuresOfRow, nearest, fitVec,
              nnls, fitChild, fitAll, report, setExtra: (f) => { EXTRA = f; } };
/* THE CLI WRITES NOTHING UNLESS ASKED. The deleted original wrote
   nukernel/GENEALOGY.md on every run; that file is not in the tree today and
   re-creating it as a side effect of `node tools/genealogy.js` would be this
   tool deciding for a person. It prints; `--write <path>` saves. */
if (NODE && require.main === module) {
  const md = report();
  console.log(md);
  const i = process.argv.indexOf("--write");
  if (i >= 0 && process.argv[i + 1]) {
    require("fs").writeFileSync(process.argv[i + 1], md + "\n");
