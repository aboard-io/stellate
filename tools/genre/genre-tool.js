...
function buildVocab() {
  ...
  const stateEngineSrc = fs.readFileSync(path.join(ROOT, "faust", "voices", "state-engine.js"), "utf8");
...
function splicePosition(name, pos) {
  // POS lives in app/core/world.js (a BAKED cache of
  ...
  // on the fast path.
  if (pos == null) { console.log(`  pos: none given — boot will derive a star near ${name}'s musical family; re-bake app/core/world.js POS after the batch`); return; }
  ...
  const file = path.join(__dirname, "..", "..", "app", "core", "world.js");
  let src = fs.readFileSync(file, "utf8");
  const OPEN = "export const POS={", CLOSE = "\n};";
  if (src.indexOf(OPEN) < 0) die("POS table not found in app/core/world.js");
...
// The star positions live in app/core/world.js as a baked ES-module literal; parse
// rather than import so this stays a plain CJS tool.
const POS_TABLE = (() => {
  try {
    const src = fs.readFileSync(path.join(__dirname, "..", "..", "app", "core", "world.js"), "utf8");
...
function cmdInit() {
  ...
  console.log(`  next     : edit label/info + the anchor, then  node tools/genre/genre-tool.js create genre-specs/${name}.json --dry-run`);
}

// ============================================================ exports
// The measure -> derive -> serialize -> splice machinery is reused by
// tools/genre/invent-genres.js (the gap-finding invention pipeline), so it requires
