...
// (tools/corpus-db.js), in exactly the shape theory.js's functional walk
// consumes: FUNC_NEXT (T/S/D 3x3) + POOL (degree weights per function), one
// set per mode family (major / minor).
//
//   node tools/mine-theory.js [--db path] [--min-margin 0.01] [--json out] [--splice]
//
...
//      (test/theory-tables.test.js re-runs this claim against the stored
//      per-mode metrics.)
//
...
function main() {
  ...
  if (argv.includes("--splice")) {
    ...
    const BEGIN = "  // ---- MINED-TABLES BEGIN (tools/mine-theory.js — do not hand-edit) ----";
