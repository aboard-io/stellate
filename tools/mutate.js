#!/usr/bin/env node
// tools/mutate.js — ROADMAP §1.3.5 — the mutation / breeder.
//
...
//
// CLI:
//   node tools/mutate.js <genre> <dim> <delta> [opts]
//   node tools/mutate.js techno bpm +10
//   node tools/mutate.js vaporwave swing +0.2 --seed 7
//   node tools/mutate.js techno bpm 0            # determinism guard (no-op)
//   node tools/mutate.js jungle fx.delayFb +0.3 --mode add --json
// opts:
...
function main(argv) {
  ...
  if (pos.length < 2) {
    console.error('usage: node tools/mutate.js <genre> <dim> [delta] [--seed N] [--seeds a,b,c] [--mode add|scale|set] [--audit-all] [--json]');
    process.exit(2);
  }
  opt.dim = pos[1];
